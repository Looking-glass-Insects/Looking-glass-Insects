...
public class guidao3 extends BaseOrbitController {
...
//    protected float start_x;
    ...
    public void addItem(float x,float y) {
        BaseBigLightBullet b = new BaseBigLightBullet(){
            @Override
...
//                    iterator.remove();
            ...
            item.speedY=0.7f;
            item.speedX = 0.7f;
             item.setPosition(point.x-item.speedX,-4*b1*(point.x-a1/2)*(point.x-a1/2)/(a1*a1)+2*b1);
        }
    }
